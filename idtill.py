...
def main(): 
    ...
    # Start RFID thread
    threading.Thread(target=rfid_thread, args = (uid_queue,), daemon = True).start()

    # Start Database Thread 
    threading.Thread(target=database_thread, args = (uid_queue, gui_queue,), daemon = True).start()

    # Start GUI
    app = QtWidgets.QApplication([])
    window = ThesisGUI(gui_queue)
